...
import order.service.AddOrderDetailtRequest;
import order.service.AddOrderService;
import order.service.AddPaymentRequest;
import order.service.AddRequest;

public class AddReceiptHandler implements CommandHandler {//웹요청처리
	...
	private String processSubmit(HttpServletRequest req, HttpServletResponse res) throws Exception {// 폼 전송 처리
		...
		AddPaymentRequest payreq = new AddPaymentRequest();
		payreq.setTotalsale(Integer.parseInt(req.getParameter("totalsale")));
		payreq.setDiscount(Integer.parseInt(req.getParameter("discount")));
		payreq.setCheck(Integer.parseInt(req.getParameter("check")));
		payreq.setRevenue(Integer.parseInt(req.getParameter("revenue")));
		payreq.setChange(Integer.parseInt(req.getParameter("change")));
